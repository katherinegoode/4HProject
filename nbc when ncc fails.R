...
patientprediction <- function(prediction){
  ...
  nbcfic <- c()
  for (i in(1:80)){
    nccfic <- append(nccfic, sum(as.numeric(as.character(prediction$`NCCAccurate?`[(3*i-2):(3*i)]))))
    nbcfic <- append(nbcfic, sum(as.numeric(as.character(prediction$`NBCAccurate?`[(3*i-2):(3*i)]))))
  }
  for (i in (1:80)){
    if(nbcfic[i] == 3){
      nbcfic[i] = 'correct'
  ...
  predictions <- as.data.frame(cbind(actclass, nccfic, nbcfic))
  names(predictions)[1] <- 'class'
  names(predictions)[2] <- 'NCCPredictionAccuracy'
  names(predictions)[3] <- 'NBCPredictionAccuracy'
  indet <- predictions[which(predictions[,2]=='indeterminate'),]
  numindet <- dim(indet)[1]
  corr <- indet[which(indet[,3] == 'correct'),]
  numcorr <- dim(corr)[1]
...
nccvcnbc <- c()
for(i in 1:45){
  nccvcnbc <- append(nccvcnbc, patientprediction(crossvalidatecredal(mydata, 1)))
}
...
patientprediction(comp1)
comp1
comp1 <- crossvalidatecredal(mydata,1)

nbcacc <- function(predtable){
  corr <- length(which(predtable[,7] ==1))
  tot <- dim(predtable)[1]
  return(c(corr/tot, tot))
}
...
totals <- c()
for(i in 1:4){
  prd <- nbcacc(crossvalidatecredal(mydata,1))
  nccvcnbc <- append(nccvcnbc, prd[1])
  totals <- append(totals, prd[2])
}


mean(nccvcnbc[-c(11, 20, 32, 34)])
...
par(mfrow = c(1,2))
plot(svalues, credalsingleacc, main = "Single Accuracy", xlab = "s", ylab = "%", type = "l", col = "blue")
plot(svalues, credaldeterminacy, main = "Determinacy", xlab = "s", ylab = "%", type = "l", col = "red")
