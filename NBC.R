...
g <- function(data, case){
  k <- length(case)
  g0 <- ((n_c(data)[1])^(1-k))*prod(n_aiccase(data, case, 0))
  g1 <- ((n_c(data)[2])^(1-k))*prod(n_aiccase(data, case, 1))
  return(g0 - g1)
}
...
g <- function(data, case){
  k <- length(case)
  numerator0 <- n_aiccase(data, case, 0)
  numerator0[1] <- numerator0[1] + 0.125
  numerator0 <- numerator0 + 0.125
  numerator1 <- n_aiccase(data, case, 1)
  numerator1[1] <- numerator1[1] + 0.125
  numerator1 <- numerator1 + 0.125
  g0 <- ((n_c(data)[1] + 0.5)^(1-k))*prod(numerator0)
  g1 <- ((n_c(data)[2] + 0.5)^(1-k))*prod(numerator1)
  return(g0 - g1)
}
